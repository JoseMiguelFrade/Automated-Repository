...
def extract_text_from_pdf_with_pdfminer(pdf_path="", pdf="",max_tokens=850):
    if pdf_path:
    ...
    truncated_text = ""
    for word in words:
        if num_tokens_from_string(truncated_text+word,"gpt-4") > max_tokens:
            break
        truncated_text += word + " "
...
def analyze_document(pdf_path, total_queries, gpt_3_5_count):
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            ...
            {"role": "user", "content": f"Analyze the following document extract: '{pdf_text}'. First, determine if it is related to IT/cybersecurity/data privacy/AI. If it is not related, respond with 'is_related:<no>'. If it is related, provide structured information with the following format (if no related_docs, the value for related_docs is <none>) (use English to write the Abstract and Type): 'is_related:<yes>#issuer:<issuer_name>#origin:<origin>#type:<Norm/Law/Regulation/Technichal Guide>#subject:<Privacy/Governance/Cyberecurity>#date:<date in dd/mm/yyyy format>#area:<Finance/Healthcare/General/Energy/...>#title:<document_title>#Related_Docs:doc1|doc2|doc3#abstract:<brief_summary (95 tokens max)>'."}
        ],
        temperature=0.4,
        max_tokens=380,
        top_p=1
...
def regenerate_document_field(pdf, field, temperature):
    ...
    if field == 'title':
        ...
        user_content = f"Make an abstract (95 tokens max) based on the following document extract: '{pdf_text}'. Plese answer in the format 'abstract:<brief_summary>'."
        ...
        user_content = f"What is the type (law/norm/regulation/treaty/Technical Guide/...) of the following document extract: '{pdf_text}'. Plese answer in the format 'type:<type>'."
    elif field == 'subject':
