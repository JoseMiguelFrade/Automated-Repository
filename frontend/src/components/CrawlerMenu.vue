    ...
    <v-row justify="center">
      <v-col cols="12" sm="8" md="6">
        <v-card class="pa-4" outlined elevation="2">
          <v-row>
            <v-col cols="12">
              <v-text-field label="Enter URL or Upload a List" v-model="inputUrl" @input="clearFileSelection" outlined
                :append-inner-icon="'mdi-upload'" @click:append-inner="openFileUpload"></v-text-field>
          ...
          <v-row>
            <v-col cols="12">
              <v-select label="Select Depth" :items="[1, 2, 3, 4]" v-model="depth" outlined></v-select>
            </v-col>
          </v-row>
  ...
  methods: {
    ...
    launchCrawler() {
      ...
      if (urlsToCrawl.length) {
        axios.post(`${apiUrl}/start-crawler`, { urls: urlsToCrawl, depth: this.depth })
