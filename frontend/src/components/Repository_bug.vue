  ...
  export default {
    ...
    data() {
      return {
        ...
        originOptions: ['European Union', 'European Commission','Portugal','Other'],
    ...
    applyFiltersToDocument(doc) {
      const matchesOther = (categoryOptions, docValues) => {
        return !categoryOptions.includes(docValues) && docValues !== undefined;
      };
  
      const matchesCategory = (docValue, filterValues, categoryOptions) => {
        if (!filterValues.length) return true; // No filters applied
        return filterValues.includes(docValue) || (filterValues.includes('Other') && matchesOther(categoryOptions, docValue));
      };
  
      return matchesCategory(doc.type, this.filters.type, this.typeOptions) &&
             this.matchesIssuerAlias(doc.issuer, this.filters.issuer) &&
             matchesCategory(doc.origin, this.filters.origin, this.originOptions) &&
             matchesCategory(doc.subject, this.filters.subject, this.subjectOptions) &&
             matchesCategory(doc.area, this.filters.area, this.areaOptions);
    },
    ...
    matchesIssuerAlias(issuer, filterIssuers) {
      ...
      if (isOtherSelected && !filterIssuers.some(filterIssuer => this.issuerAliases[filterIssuer]?.includes(issuer))) {
        return !this.issuerOptions.includes(issuer);
      }
      
      return filterIssuers.some(filterIssuer => 
        this.issuerAliases[filterIssuer]?.includes(issuer) || filterIssuer === issuer
      );
    },
